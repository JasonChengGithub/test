...
$("body").on('click', '.uploadFile .inputFile', function () {
	var dataName = $(this).parents(".uploadFile").attr("data-name");
...
$("body").on('change', '.uploadFile .inputFile', function () {
	...
	$.ajaxFileUpload({
		url: l.domain + 'uploadFiles.do', //用于文件上传的服务器端请求地址
...
function updateUploadList(dataName, _uploadList, disabled) {
	...
	if (disabled && !_uploadList.length > 0) {
		...
		$.each(_uploadList, function (i, v) {
			$(".uploadFile[data-name=" + dataName + "] .uploadList").append("<li class='uploadItem'><a target='_blank' href='" + v.fileUrl + "'>" + v.fileName + "</a>" + (disabled ? "" : "<i data-index='" + i + "' class='uploadRemove icon Hui-iconfont pl-10 pr-10'></i>") + "</li>")
		})
	}
